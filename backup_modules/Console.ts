...
export class ConsoleManager {
  ...
  private async saveConsoleState(context: ScriptContext, state: ConsoleState): Promise<void> {
  ...
  private async loadConsoleStates(context: ScriptContext): Promise<void> {
    ...
    const iterator = context.kv.list<ConsoleState>({ prefix: ['consoles', 'state'] });
    ...
    for (const state of states) {
      try {
        ...
        await context.display.log(state.id, `Console restored at ${new Date().toISOString()}`);
        await context.display.log(state.id, `Owner: ${state.owner}, Status: ${state.isPublic ? 'Public' : 'Private'}`);
  ...
  async handleServerStart(context: ScriptContext): Promise<void> {
    ...
    try {
      ...
      await context.api.executeCommand('kill @e[type=text_display,tag=display_console]');
  ...
  @Permission('operator')
  @Argument([{ name: 'id', type: 'string', description: 'Unique identifier for the console' },
    { name: 'public', type: 'string', description: 'Whether the console is publicly viewable' }])
  async createConsole({ params, kv, api, display, log }: ScriptContext): Promise<void> {
    ...
    log(`Creating console ${id} at position ${consolePos.x}, ${consolePos.y}, ${consolePos.z}`);
  ...
  @Permission('operator')
  @Argument([{ name: 'id', type: 'string', description: 'Console identifier to remove' }])
  async removeConsole({ params, kv, api, display }: ScriptContext): Promise<void> {
    const { args, sender } = params;
    const { id } = args;
    ...
    await api.tellraw(sender, `{"text":"Console ${id} removed","color":"green"}`);
  }
  ...
  async listConsoles({ params, api }: ScriptContext): Promise<void> {
    ...
    const consoles = Array.from(this.consoleStates.values())
      ...
      .map(state => `${state.id} (Owner: ${state.owner}, ${state.isPublic ? 'Public' : 'Private'})`);

    if (consoles.length === 0) {
      await api.tellraw(sender, `{"text":"No consoles available","color":"yellow"}`);
      return;
    }
  ...
  @Permission('player')
  @Argument([{ name: 'id', type: 'string', description: 'Console identifier'}, { name: 'message', type: 'string', description: 'Message to log' }])
  ...
  @Permission('player')
  @Argument([{ name: 'id', type: 'string', description: 'Console identifier to clear' }])
  ...
  @Permission('operator')
  @Argument([{ name: 'id', type: 'string', description: 'Console identifier to move' }])
  async moveConsole({ params, api, kv, display, log }: ScriptContext): Promise<void> {
    const { args, sender } = params;
    const { id } = args;
    const state = this.consoleStates.get(id);
    ...
    log(`Moving console ${id} to position ${newPos.x}, ${newPos.y}, ${newPos.z}`);
  ...
  async resizeConsole({ params, display, kv, log }: ScriptContext): Promise<void> {
    const { args, sender } = params;
  ...
  @Event('player_joined')
  async handlePlayerJoin({ params, display, log }: ScriptContext): Promise<void> {
  ...
  @Event('player_left')
  async handlePlayerLeave({ params, display, log }: ScriptContext): Promise<void> {
    for (const [id, state] of this.consoleStates.entries()) {
      if (state.isPublic) {
  ...
  @Event('player_death')
  async handlePlayerDeath({ params, display, log }: ScriptContext): Promise<void> {
    for (const [id, state] of this.consoleStates.entries()) {
      if (state.isPublic) {
