  ...
  async send(
    ...
    try {
      ...
      if (totalCost > senderBalance) {
        throw new Error(
          `Insufficient balance. Required: ${totalCost} XPL (including ${fee} XPL fee)`,
        );
      }
      ...
      await kv.set(
        ["plugins", "economy", "balances", sender],
        new Deno.KvU64(BigInt(newSenderBalance)),
      );
      await kv.set(
        ["plugins", "economy", "balances", recipient],
        new Deno.KvU64(BigInt(newRecipientBalance)),
      );
      ...
      // Notify sender
      const senderMessages = await tellraw(
        sender,
        container([
          alert([
      ...
      // Notify recipient
      const recipientMessages = await tellraw(
        recipient,
        alert([
          text("You received "),
          this.renderCurrency(amount),
      ...
      return {
        messages: [...senderMessages, ...recipientMessages],
        success: true,
      };
    } catch (error) {
      const messages = await tellraw(
