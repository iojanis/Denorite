...
export class Teams {
    ...
    try {
      messages = await tellraw(
        ...
        JSON.stringify([
          ...
          {
            text:
              ` - Create a new team (costs ${this.TEAM_CREATION_COST} XPL)\n`,
            color: "gray",
          },
  ...
  ): Promise<{ messages: any[]; success?: boolean; team?: TeamData }> {
  ...
  async teamInfo(
    { params, kv, tellraw, log }: ScriptContext,
  ): Promise<{ messages: any[]; team?: TeamData }> {
    const { sender } = params;
    let messages = [];
    ...
    try {
      ...
      const result = await kv.atomic()
        ...
        .set(
          ...
          new Deno.KvU64(BigInt(playerBalance - args.amount)),
        )
        .set(["teams", teamId], {
          ...team,
  ...
  async withdrawFromTeam(
    ...
    try {
      ...
      if (team.balance < args.amount) {
        throw new Error(
          `Insufficient team funds. Available: ${team.balance} XPL`,
        );
      }
  ...
  async transferLeadership(
    ...
    try {
      ...
      log(
        `${sender} transferred team ${team.name} leadership to ${args.player}`,
      );
      return { messages, success: true };
    } catch (error) {
