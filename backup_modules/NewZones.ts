...
export class Zones {
    ...
    try {
      messages = await tellraw(sender, JSON.stringify([
        ...
        {text: ` - Create a new zone (costs ${this.ZONE_COST} XPL)\n`, color: "gray"},
        {
          ...
          hoverEvent: {action: "show_text", value: "Click to list your team's zones"}
        },
  ...
  async createZone({ params, kv, tellraw, log, rcon, api, bluemap }: ScriptContext): Promise<{ messages: any[], success?: boolean, zone?: Zone }> {
  ...
  async zoneInfo({ params, kv, tellraw, log, rcon, bluemap }: ScriptContext): Promise<{ messages: any[], zone?: Zone }> {
    const { sender, args } = params;
    let messages = [];
    ...
    try {
      ...
      if (args.zoneId) {
        ...
        currentZone = zones.find(zone => this.isCoordinateInZone(position, zone)) || null;
  ...
  @Permission('player')
  async listZones({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[], zones?: Zone[] }> {
    const { sender } = params;
    let messages = [];
      ...
      messages = await tellraw(sender, JSON.stringify([
        ...
        ...teamZones.flatMap(zone => [
          ...
          {text: `\nDescription: ${zone.description}`, color: "white"},
          {
            text: "\n[Teleport]",
            color: "aqua",
            clickEvent: {action: "run_command", value: `/zones tp ${zone.id}`}
          }
        ])
      ]));
  ...
  async modifyZone({ params, kv, tellraw, log, rcon }: ScriptContext): Promise<{ messages: any[], success?: boolean, zone?: Zone }> {
    const { sender, args } = params;
    let messages = [];
    ...
    try {
      ...
      log(`Zone ${zone.name} modified by ${sender}: ${args.setting} = ${args.value}`);
  ...
  async deleteZone({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[] }> {
    const { sender, args } = params;
    let messages = [];
    ...
    try {
      ...
      messages = await tellraw(sender, JSON.stringify([
        ...
        {text: "\n\nThis confirmation will expire in 60 seconds.", color: "gray", italic: true}
      ]));
  ...
  async confirmDeleteZone({ params, kv, tellraw, log, rcon, bluemap }: ScriptContext): Promise<{ messages: any[], success?: boolean }> {
    const { sender, args } = params;
    let messages = [];

    try {
      // Check pending deletion
      const pendingResult = await kv.get(['pending_deletions', sender, args.zoneId]);
      if (!pendingResult.value) {
        throw new Error('No pending deletion found or confirmation expired. Please start the deletion process again.');
      }
      ...
      for (const member of team.members) {
        if (member !== sender) {
          await tellraw(member, JSON.stringify([
            {text: "Zone Deleted\n", color: "red", bold: true},
            {text: zone.name, color: "yellow"},
  ...
  async teleportToZone({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[], success?: boolean }> {
    const { sender, args } = params;
    let messages = [];
    ...
    try {
      ...
      messages = await tellraw(sender, JSON.stringify([
        ...
        {text: "\nLocation: ", color: "gray"},
        {
          text: `${Math.floor(zone.center.x)}, ${Math.floor(zone.center.y)}, ${Math.floor(zone.center.z)}`,
          color: "aqua",
          clickEvent: {
  ...
  private async teleportPlayer(player: string, position: Position): Promise<void> {
  ...
  @Event('player_moved')
  async handlePlayerMovement({ params, kv, tellraw }: ScriptContext): Promise<{ messages: any[] }> {
