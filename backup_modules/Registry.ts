...
export class PackageManager {
  ...
  private async fetchFromRegistry(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.debug(
          `Registry request failed: ${response.status} - ${errorText}`,
        );
        throw new Error(
  ...
  @Argument([
    {
      name: "type",
      type: "string",
      description: "Type of package (module/component)",
    },
    {
      name: "path",
      type: "string",
      description: "Package path (e.g., userspace/name or just name)",
    },
  ])
  async install({ params, api, log, kv }: ScriptContext): Promise<void> {
    ...
    const { userspace, name } = this.parsePackagePath(
      args.path,
      config.defaultUserspace,
    );
    ...
    try {
      ...
      if (installed.value) {
        ...
        throw new Error(
          `Package ${name} is already installed (v${record.version} from ${record.userspace})`,
        );
      }

      const response = await this.fetchFromRegistry(
        `/${userspace}/${type}s/${name}`,
        config,
      );
      if (!response.ok) {
      ...
      try {
        ...
        await api.tellraw(
          ...
          JSON.stringify({
            text:
              `Successfully installed ${type} ${userspace}/${name} v${pkg.version}`,
            color: "green",
          }),
        );
        ...
        if (backup) {
          ...
          throw new Error(
            `Installation failed, rolled back to previous version: ${error.message}`,
          );
        }
        throw error;
      }
    ...
    try {
      ...
      try {
        ...
        if (backup) {
          await this.savePackage(kv, backup, type as "module" | "component");
          throw new Error(
            `Uninstall failed, rolled back to previous state: ${error.message}`,
          );
        }
        throw error;
      }
    ...
    try {
      ...
      try {
        ...
        await api.tellraw(
          ...
          JSON.stringify({
            text:
              `Updated ${type} ${name} from v${record.version} to v${pkg.version}`,
            color: "green",
          }),
        );
        ...
        if (backup) {
          await this.savePackage(kv, backup, type as "module" | "component");
          throw new Error(
            `Update failed, rolled back to v${record.version}: ${error.message}`,
          );
        }
        throw error;
      }
  ...
  @Argument([
    ...
    },
    {
      name: "userspace",
      type: "string",
      description: "Filter by userspace",
      optional: true,
    },
  ])
  async packages({ params, api, log, kv }: ScriptContext): Promise<void> {
    ...
    try {
      ...
      const responses = await Promise.all([
        type !== "component"
          ? this.fetchFromRegistry("/public/modules", config)
          : null,
        type !== "module"
          ? this.fetchFromRegistry("/public/components", config)
          : null,
  ...
  async publish({ params, api, log, kv }: ScriptContext): Promise<void> {
    ...
    try {
      ...
      await api.tellraw(
        ...
        JSON.stringify({
          text:
            `Successfully published ${type} ${name} v${version} to ${userspace}`,
          color: "green",
        }),
      );
