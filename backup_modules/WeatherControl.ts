...
export class WeatherControl {
  ...
  private async setState(
    kv: any,
    newState: Partial<WeatherControlState>,
  ): Promise<void> {
  ...
  private pickNextWeather(
    settings: WeatherControlState["cycleSettings"],
  ): "clear" | "rain" | "thunder" {
    const totalWeight = settings.clear.weight + settings.rain.weight +
      settings.thunder.weight;
  ...
  async setWeather({ params, kv, api }: ScriptContext): Promise<void> {
    ...
    try {
      ...
      await api.tellraw(
        sender,
        JSON.stringify({
          text: `Weather set to ${weatherType} for ${
      ...
      if (!args.action) {
        ...
        await api.tellraw(
          ...
          JSON.stringify([
            ...
            {
              text: `Custom Cycle: ${
                state.customCycle ? "Enabled" : "Disabled"
              }\n\n`,
            ...
            {
              text: `Duration: ${
                this.formatDuration(state.cycleSettings.clear.minDuration)
              } - ${
                this.formatDuration(state.cycleSettings.clear.maxDuration)
              }\n`,
              color: "yellow",
            },
            {
            ...
            {
              text: `Duration: ${
                this.formatDuration(state.cycleSettings.rain.minDuration)
              } - ${
                this.formatDuration(state.cycleSettings.rain.maxDuration)
              }\n`,
              color: "aqua",
            },
            {
            ...
            {
              text: `Duration: ${
                this.formatDuration(state.cycleSettings.thunder.minDuration)
              } - ${
                this.formatDuration(state.cycleSettings.thunder.maxDuration)
              }\n`,
              color: "blue",
            },
            {
          ...
          if (!type || !setting || isNaN(value)) {
            await api.tellraw(
              ...
              JSON.stringify([
                {
                  text:
                    "Usage: /weather cycle config <type> <setting> <value>\n",
                  color: "yellow",
                },
                {
    ...
    try {
      ...
      await api.tellraw(
        ...
        JSON.stringify([
          ...
          {
            text: `Time Since Last Change: ${
              this.formatDuration(Math.floor((now - state.lastChange) / 1000))
            }\n`,
            color: "aqua",
          },
          {
            text: `Time Until Next Change: ${
              this.formatDuration(Math.floor(timeUntilNext))
            }\n`,
            color: "aqua",
          },
          {
