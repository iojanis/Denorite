...
export async function getEntityData(api: ScriptContext['api'], target: string, path?: string): Promise<Record<string, unknown> | null> {
  const command = path
...
export class EnchantmentModuleManager {
  ...
  private async getEnchantmentModules(): Promise<EnchantmentModule[]> {
    ...
    for await (const entry of Deno.readDir(this.ENCHANTMENTS_DIR)) {
      if (entry.isFile && extname(entry.name) === '.ts') {
        const content = await Deno.readTextFile(join(this.ENCHANTMENTS_DIR, entry.name));
        const title = entry.name.replace('.ts', '');
        modules.push({
          title,
          author: 'Unknown', // We can't reliably get the author from the file
          content,
          createdAt: (await Deno.stat(join(this.ENCHANTMENTS_DIR, entry.name))).birthtime?.toISOString() || new Date().toISOString(),
        });
      }
    }
    return modules;
  ...
  private async getHeldBook(api: ScriptContext['api'], player: string): Promise<EnchantmentModule | null> {
    try {
  ...
  @Description('Upload the writable book you are holding as an enchantment module')
  @Permission('player')
  @Socket()
    ...
    try {
      ...
      if (!heldBook) {
        await api.tellraw(sender, JSON.stringify({
          text: "You must be holding a writable book to upload it as an enchantment module.",
          color: "red"
        }));
      ...
      await api.tellraw(sender, JSON.stringify({
        text: `Successfully uploaded "${heldBook.title}" as an enchantment module.`,
        color: "green"
      }));
  ...
  @Socket()
  @Argument([
    { name: 'title', type: 'string', description: 'The title of the enchantment module to read' }
  ])
  async readEnchantmentModule({ params, api, log }: ScriptContext) {
    ...
    try {
      ...
      log(`Error reading enchantment module "${title}" for ${sender}: ${error}`);
      await api.tellraw(sender, JSON.stringify({
  ...
  @Description('Download an enchantment module to your inventory as a written book')
  @Permission('player')
  @Socket()
  @Argument([
    { name: 'title', type: 'string', description: 'The title of the enchantment module to download' }
  ])
  async downloadEnchantmentModule({ params, api, log }: ScriptContext) {
    ...
    try {
      ...
      await api.tellraw(sender, JSON.stringify({
        text: `Successfully downloaded "${title}" to your inventory as a written book.`,
        color: "green"
      }));
      log(`${sender} downloaded enchantment module "${title}"`);
      return { success: true, module: { title, content } };
    } catch (error) {
      log(`Error downloading enchantment module "${title}" for ${sender}: ${error}`);
      await api.tellraw(sender, JSON.stringify({
