...
export class PlayerManager {
  ...
  async loadPlayerData(playerName: string): Promise<EnhancedPlayerData | null> {
    try {
      ...
      this.logger.error(`Error loading player data for ${playerName}: ${error}`);
      return null;
    }
  }
  ...
  async handleMinecraftConnection(data: {
    ...
    let playerData = this.players.get(playerName) || await this.loadPlayerData(playerName);
    if (!playerData) {
    ...
    this.logger.info(`Minecraft connection established for ${playerName} (${playerData.role})`);
    return connectionId;
  }
  ...
  async updatePlayerRole(playerName: string, newRole: 'guest' | 'player' | 'operator'): Promise<boolean> {
    try {
      const playerData = this.players.get(playerName);
      ...
      await this.kv.atomic()
        ...
        .set(['player', playerName, 'permissionLevel'], playerData.permissionLevel)
        .commit();
  ...
  isConnectedVia(playerName: string, connectionType: 'minecraft' | 'websocket'): boolean {
    const player = this.players.get(playerName);
  ...
  getPlayerByConnection(connectionId: string): EnhancedPlayerData | { name: null } {
