...
export class EventLogger {
  ...
  private async cleanupOldEvents(context: ScriptContext, eventType: string): Promise<void> {
    const { kv, log } = context;

    try {
      // Get all events of this type
      const events = kv.list<EventLog>({ prefix: ['events', 'byType', eventType] });
      ...
      for (const event of toDelete) {
        ...
        if (event.value.data.playerName) {
          await kv.delete(['events', 'byPlayer', event.value.data.playerName, event.key[3]]);
        }
      }
  ...
  static async getRecentEvents(kv: Deno.Kv, limit: number = 100): Promise<EventLog[]> {
  ...
  static async getPlayerEvents(kv: Deno.Kv, playerName: string, limit: number = 100): Promise<EventLog[]> {
    const events: EventLog[] = [];
    const iterator = kv.list<EventLog>({ prefix: ['events', 'byPlayer', playerName] });
  ...
  static async getEventsByType(kv: Deno.Kv, eventType: string, limit: number = 100): Promise<EventLog[]> {
    const events: EventLog[] = [];
    const iterator = kv.list<EventLog>({ prefix: ['events', 'byType', eventType] });
