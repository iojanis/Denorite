import { Module, Command, Socket, Permission, Argument, Description, Event } from '../decorators.ts';
...
export class Bcon {
  ...
  async sendDirectMessage({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[] }> {
    ...
    try {
      ...
      if (message.length > this.MESSAGE_LENGTH_LIMIT) {
        throw new Error(`Message too long (max ${this.MESSAGE_LENGTH_LIMIT} characters)`);
      }
      ...
      await this.updateChatHistory(kv, ['bcon', 'dm', sender, recipient], newMessage);
      // Update recipient's inbox
      await this.updateChatHistory(kv, ['bcon', 'dm', recipient, sender], newMessage);
      ...
      await tellraw(recipient, recipientMsg.render({ platform: 'minecraft', player: recipient }));
      ...
      const messages = await tellraw(sender, senderMsg.render({ platform: 'minecraft', player: sender }));
      return { messages };
  ...
  async sendGlobalMessage({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[] }> {
    const { sender, args } = params;
    const { message } = args;
    ...
    try {
      ...
      const messages = await tellraw('@a', globalMsg.render({ platform: 'minecraft', player: '@a' }));
      return { messages };
  ...
  @Permission('player')
  @Argument([
    { name: 'player', type: 'player', description: 'Player to view history with' }
  ])
  async viewHistory({ params, kv, tellraw }: ScriptContext): Promise<{ messages: any[] }> {
    const { sender, args } = params;
    const { player } = args;
    ...
    try {
      ...
      const historyDisplay = container([
        ...
        ...history.messages.map(msg => [
          text(`${new Date(msg.timestamp).toLocaleTimeString()} `, { style: { color: 'gray' }}),
      ...
      const messages = await tellraw(sender, historyDisplay.render({ platform: 'minecraft', player: sender }));
      return { messages };
  ...
  private async updateChatHistory(kv: any, key: string[], newMessage: Message): Promise<void> {
  ...
  async getChatHistory({ params, kv }: ScriptContext): Promise<any> {
    try {
      ...
      const key = type === 'global' ? ['bcon', 'global'] : ['bcon', 'dm', params.sender, player];
    ...
    try {
      ...
      for await (const entry of dmIterator) {
        ...
        if (history.messages.length > 0) {
          ...
          if (lastMessage.sender !== sender && (!history.lastRead || lastMessage.timestamp > history.lastRead)) {
            totalUnread++;
          }
        }
      }
