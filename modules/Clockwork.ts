...
export class Clockwork {
  ...
  private isNightTime(time: number): boolean {
    ...
    return normalizedTime >= this.NIGHT_START &&
      normalizedTime < this.DAY_START + this.FULL_DAY;
  }
  ...
  async handleServerTick({ api, kv }: ScriptContext): Promise<void> {
    try {
      ...
      const lastCommandTime = await this.getConfig(
        kv,
        "lastCommandTime",
        Date.now(),
      );
      ...
      const currentSpeed = this.isNightTime(currentTime)
        ? nightSpeed
        : daySpeed;
  ...
  @Permission("player")
  async showStatus(
    { kv, api, tellraw, params }: ScriptContext,
  ): Promise<{ messages: any[] }> {
    const { sender } = params;
    ...
    try {
      ...
      const statusDisplay = container([
        ...
        text(`${this.formatDuration(Date.now() - lastCommandTime)} ago`, {
          style: { color: "yellow" },
        }),
  ...
  async setTime(
    { params, kv, api, tellraw }: ScriptContext,
  ): Promise<{ messages: any[] }> {
    const { sender, args } = params;
    ...
    try {
      ...
      switch (args.time.toLowerCase()) {
        ...
        default:
          ...
          if (
            isNaN(targetTime) || targetTime < 0 || targetTime >= this.FULL_DAY
          ) {
            throw new Error(
              "Invalid time value. Use 0-24000 or day/noon/night/midnight",
            );
          }
      }
