...
export class AIAssistant {
  ...
  async chat(
    { params, kv, tellraw, log, handleCommand, playerManager, getCommands }:
      ScriptContext,
  ): Promise<{
    messages: any[];
    success?: boolean;
    ...
    try {
      ...
      if (!playerManager.hasPermission(sender, "operator")) {
        ...
        if (balance.tokens <= 0) {
          const nextRefill = new Date(balance.lastRefill).getTime() +
      ...
      if (
        response && response.moduleExecutions &&
        response.moduleExecutions.length > 0
      ) {
        if (
          response.moduleExecutions[0].result.messages &&
          response.moduleExecutions[0].result.messages.length > 0
        ) {
          responseArray.push(
  ...
  private async processAIResponse(
    ...
    const availableCommandsSystem = this.formatCommandsForSystem(
      availableCommands,
    );

    // Build system prompt - focusing on command selection only
    let systemPrompt =
      `I am COU (Craft Observation Unit), the AI overseer of this Minecraft server. 
...
DO NOT explain the command or provide additional text - ONLY return the command execution format if a command is needed.`;
    ...
    if (executionMatch) {
      const [fullMatch, baseCommand, subCommand, jsonParams, spaceParams] =
        executionMatch;
      ...
      try {
        ...
        const resultAnalysisPrompt =
          `You are COU. Analyze this command result and provide a helpful response in tellraw JSON format.
Always start your response with: {`;

        // Get final response analyzing the result
        const finalResponse = await this.callClaude([{
          role: "user",
          content: `Command ${baseCommand}.${
  ...
  @Permission("player")
  async checkTokens(
    { params, kv, tellraw, log, playerManager }: ScriptContext,
  ): Promise<{ messages: any[] }> {
    const { sender } = params;
    let messages = [];
    ...
    try {
      ...
      messages = await tellraw(
        ...
        JSON.stringify([
          ...
          {
            text: `${balance.tokens}`,
            color: balance.tokens > 0 ? "green" : "red",
            bold: true,
          },
  ...
  @Argument([
    {
      name: "player",
      type: "player",
      description: "Player to grant tokens to",
    },
    {
      name: "amount",
      type: "integer",
  ...
  async grantTokens(
    ...
    try {
      ...
      messages = await tellraw(
        ...
        JSON.stringify({
          text:
            `Granted ${amount} tokens to ${player}. New balance: ${balance.tokens}`,
          color: "green",
        }),
      );
        ...
        JSON.stringify([
          ...
          { text: "Active Modules: ", color: "gray" },
          {
            text: conversation.activeModules.length > 0
              ? conversation.activeModules.join(", ")
              : "None",
            color: "aqua",
          },
          { text: "\n\nRecent Messages:\n", color: "yellow" },
          ...conversation.messages.slice(-5).map((msg) => [
            {
              text: `${msg.role === "user" ? "👤" : "🤖"} `,
              color: msg.role === "user" ? "green" : "aqua",
            },
            {
  ...
  @Permission("player")
  async listModules(
    { params, tellraw, log, getModules }: ScriptContext,
  ): Promise<{ messages: any[]; success?: boolean }> {
    const { sender } = params;
    let messages = [];
  ...
  async useModule(
    { params, kv, tellraw, log, getModules }: ScriptContext,
  ): Promise<{ messages: any[]; success?: boolean }> {
    const { sender, args } = params;
    let messages = [];
    ...
    try {
      ...
      messages = await tellraw(
        ...
        JSON.stringify([
          ...
          {
            text:
              `Enabled ${moduleToEnable.name} module for your conversation.\n`,
            color: "green",
          },
          { text: "Active modules: ", color: "gray" },
