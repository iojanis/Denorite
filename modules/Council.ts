...
export class Council {
  ...
  private renderProgressBar(votes: number, total: number, length: number = this.PROGRESS_BAR_LENGTH): string {
  ...
  private getProgressBarColor(votes: number, total: number, minPercentage?: number): string {
  ...
  @Permission('player')
  async council({ params, tellraw }: ScriptContext): Promise<{ messages: any[] }> {
    const { sender } = params;
    let messages = [];
  ...
  async proposeVote({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[], success?: boolean, vote?: Vote }> {
  ...
  @Event('player_chat')
  async handleProposalChat({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[] }> {
    ...
    try {
      ...
      if (pending.step === 'description' && message.startsWith('desc:')) {
        ...
        if (description.length < this.MIN_DESCRIPTION_LENGTH) {
          messages = await tellraw(sender, JSON.stringify({
            text: `Description too short. Minimum ${this.MIN_DESCRIPTION_LENGTH} characters.`,
            color: "red"
          }));
          return { messages };
        }
        ...
        messages = await tellraw(sender, JSON.stringify([
          ...
          {text: "Now add voting options. Type each option in chat starting with ", color: "gray"},
        ...
        if (message.toLowerCase() === 'done') {
          ...
          messages = await tellraw(sender, JSON.stringify([
            ...
            {text: option + "\n", color: "yellow"},
            {text: `${pending.options.length}/${this.MAX_OPTIONS} options`, color: "gray"}
          ]));
        }
      }
  ...
  @Permission('player')
  async listVotes({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[], votes?: Vote[] }> {
    const { sender } = params;
    let messages = [];
    ...
    try {
      ...
      for (const vote of activeVotes) {
        ...
        const leadingOption = [...vote.options].sort((a, b) => b.votes - a.votes)[0];
        ...
        const progressBar = this.renderProgressBar(leadingOption.votes, totalVotes);
        const progressColor = this.getProgressBarColor(leadingOption.votes, totalVotes);
        ...
        messages = await tellraw(sender, JSON.stringify([
          ...
          {text: `${leadingOption.votes}/${totalVotes} votes`, color: "gray"},
  ...
  async viewVote({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[], vote?: Vote }> {
    const { sender, args } = params;
    let messages = [];
    ...
    try {
      ...
      messages = await tellraw(sender, JSON.stringify([
        ...
        {text: "Status: ", color: "gray"},
        {
          ...
          color: vote.status === 'active' ? "green" :
            vote.status === 'passed' ? "aqua" :
              vote.status === 'failed' ? "red" : "gray"
        },
      ...
      for (const option of vote.options) {
        ...
        const progressColor = this.getProgressBarColor(option.votes, totalVotes);
  ...
  async castVote({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[], success?: boolean }> {
    const { sender, args } = params;
    let messages = [];
    ...
    try {
      ...
      // Notify vote author
      await tellraw(vote.author, JSON.stringify([
  ...
  @Permission('player')
  async viewHistory({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[], votes?: Vote[] }> {
    const { sender } = params;
    let messages = [];
    ...
    try {
      ...
      for (const vote of completedVotes) {
        const totalVotes = vote.voters.size;
        const winningOption = [...vote.options].sort((a, b) => b.votes - a.votes)[0];
        const winPercentage = totalVotes > 0 ? Math.round((winningOption.votes / totalVotes) * 100) : 0;
        ...
        messages = await tellraw(sender, JSON.stringify([
          ...
          {
            text: vote.status.toUpperCase() + "\n",
            color: vote.status === 'passed' ? "green" :
              vote.status === 'failed' ? "red" : "gray"
          },
  ...
  async cancelVote({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[], success?: boolean }> {
    const { sender, args } = params;
    let messages = [];
  ...
  @Event('timer_hourly')
  async processExpiredVotes({ kv, tellraw, log }: ScriptContext): Promise<void> {
    try {
      const votes = await this.getAllVotes(kv);
      const now = new Date();
      ...
      for (const vote of votes) {
        if (vote.status === 'active' && new Date(vote.expiresAt) <= now) {
          ...
          const winPercentage = totalVotes > 0 ? (winningOption.votes / totalVotes) * 100 : 0;
          ...
          const passed = totalVotes >= requiredVotes && winPercentage >= minYesPercentage;
          vote.status = passed ? 'passed' : 'failed';

          await kv.set(['council', 'votes', vote.id], vote);

          // Announce result
          await tellraw('@a', JSON.stringify([
            {text: "📢 Vote Concluded 📢\n", color: "gold", bold: true},
            {text: vote.title + "\n", color: "yellow"},
            {text: "Result: ", color: "gray"},
            {
            ...
            {text: winningOption.text + "\n", color: "aqua"},
            {text: `${winningOption.votes}/${totalVotes} votes `, color: "yellow"},
  ...
  @Argument([
    { name: 'id', type: 'string', description: 'Vote ID' },
    { name: 'minVotes', type: 'integer', description: 'Minimum required votes' },
    { name: 'minPercentage', type: 'integer', description: 'Minimum yes percentage' }
  ])
  async setRequirements({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[], success?: boolean }> {
    const { sender, args } = params;
    let messages = [];
  ...
  @Permission('player')
  async viewStats({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[] }> {
    const { sender } = params;
    let messages = [];
    ...
    try {
      ...
      const cancelledVotes = votes.filter(v => v.status === 'cancelled').length;

      const mostVoted = votes.reduce((max, vote) =>
        vote.voters.size > (max?.voters.size || 0) ? vote : max, null as Vote | null);
