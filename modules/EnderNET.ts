...
export class EnderNET {
  ...
  @Argument([
    {
      name: "domain",
      type: "string",
      description: "Domain name (e.g. mysite.end)",
    },
  ])
  async buyDomain(
    ...
    try {
      ...
      if (!name || !tld || !this.VALID_TLDS.includes(`.${tld}`)) {
        throw new Error(
          `Invalid domain. Valid TLDs: ${this.VALID_TLDS.join(", ")}`,
        );
      }
      ...
      if (balance < this.DOMAIN_COST) {
        throw new Error(
          `Insufficient funds. Domain costs ${this.DOMAIN_COST} XPL/month`,
        );
      }
      ...
      const result = await kv.atomic()
        ...
        .set(
          ...
          new Deno.KvU64(BigInt(balance - this.DOMAIN_COST)),
        )
      ...
      const successMsg = container([
        ...
        button("Edit Content", {
          ...
          onClick: {
            ...
            value: `/net edit ${domainFull} index.tmd `,
          },
        }),
      ]);
  ...
  async visitDomain(
    ...
    try {
      ...
      const messages = await tellraw(
        sender,
        siteContainer.render({ platform: "minecraft", player: sender }),
      );
      return { messages };
    } catch (error) {
      const errorMsg = alert([], {
  ...
  async editDomain(
    ...
    try {
      if (!this.VALID_FILES.includes(file)) {
        throw new Error(
          `Invalid file. Valid files: ${this.VALID_FILES.join(", ")}`,
        );
      }

      if (content.length > this.MAX_FILE_SIZE) {
        throw new Error(
          `Content exceeds maximum length of ${this.MAX_FILE_SIZE} characters`,
        );
      }
  ...
  async netList(
    ...
    try {
      ...
      const domainList = container([
        ...
        ...domains.flatMap((domain) => [
          ...
          ...(domain.price !== null
            ? [
              ...
              button("Buy", {
                ...
                onClick: {
                  action: "run_command",
                  value: `/net purchase ${domain.name}${domain.tld}`,
                },
              }),
              text("\n"),
            ]
          ...
          button("Visit", {
            ...
            onClick: {
              action: "run_command",
              value: `/net visit ${domain.name}${domain.tld}`,
            },
          }),
          text("\n"),
          divider(),
      ...
      const messages = await tellraw(
        sender,
        domainList.render({ platform: "minecraft", player: sender }),
      );
      return { messages };
    } catch (error) {
      const errorMsg = alert([], {
  ...
  async transferDomain(
    ...
    try {
      ...
      await tellraw(
        newOwner,
        notifyMsg.render({ platform: "minecraft", player: newOwner }),
      );
  ...
  async renewDomain(
    ...
    try {
      ...
      if (balance < this.DOMAIN_COST) {
        throw new Error(
          `Insufficient funds. Renewal costs ${this.DOMAIN_COST} XPL`,
        );
      }
      ...
      const newExpiry = new Date(
        Math.max(Date.now(), currentExpiry.getTime()) +
          30 * 24 * 60 * 60 * 1000,
      );
  ...
  async listFiles(
    ...
    try {
      ...
      const fileList = container([
        ...
        ...Object.entries(domain.files).map(([filename, file]) => [
          ...
          button("View", {
            ...
            onClick: {
              action: "run_command",
              value: `/net view ${domainFull} ${filename}`,
            },
          }),
          ...(domain.owner === sender
            ? [
              ...
              button("Edit", {
                ...
                onClick: {
                  action: "suggest_command",
                  value: `/net edit ${domainFull} ${filename} `,
                },
              }),
            ]
            : []),
      ...
      const messages = await tellraw(
        sender,
        fileList.render({ platform: "minecraft", player: sender }),
      );
      return { messages };
    } catch (error) {
      const errorMsg = alert([], {
    ...
    try {
      ...
      const successMsg = container([
        ...
        text(privacy.toUpperCase(), {
          style: { color: privacy === "public" ? "green" : "red" },
        }),
      ]);
