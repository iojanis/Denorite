import { Module, Command, Permission, Argument } from '../decorators.ts';
import { ScriptContext } from '../types.ts';
import {container, text, alert, button, divider} from '../tellraw-ui.ts';
...
export class Magistry {
  ...
  private async cloneRepository(url: string, path: string): Promise<void> {
    ...
    if (!output.success) {
      throw new Error(`Failed to clone repository: ${new TextDecoder().decode(output.stderr)}`);
    }
  }
  ...
  private async pullRepository(path: string): Promise<void> {
    ...
    const output = await command.output();
    if (!output.success) {
      throw new Error(`Failed to pull repository: ${new TextDecoder().decode(output.stderr)}`);
    }
  }
  ...
  private async getModuleMetadata(filePath: string): Promise<{ name: string; version: string; description?: string } | null> {
    try {
      ...
      const versionMatch = decoratorContent.match(/version\s*:\s*['"]([^'"]+)['"]/);
      const descriptionMatch = decoratorContent.match(/description\s*:\s*['"]([^'"]+)['"]/);
  ...
  @Permission('operator')
  @Argument([
    { name: "module", type: "string", description: "Module to install in format user:repo[:Module]. If Module is omitted, installs all modules from repository" }
  ])
  async handleInstall({params, kv, tellraw}: ScriptContext): Promise<{ messages: any[] }> {
    ...
    try {
      ...
      for (const moduleName of modulesToInstall) {
        try {
          ...
          if (!metadata) {
            throw new Error(`Could not extract metadata from module ${moduleName}`);
          }
      ...
      messages = await tellraw(
        ...
        container([
          ...
          text(`Successfully installed ${modulesToInstall.length} module(s)\n`, {style: {color: 'white'}}),
          divider(),
  ...
  @Permission('operator')
  @Argument([
    { name: "repository", type: "string", description: "GitHub repository in format user:repo" }
  ])
  async handleAddRepository({params, kv, tellraw}: ScriptContext): Promise<{ messages: any[] }> {
    let messages = await tellraw(
      params.sender,
    ...
    try {
      ...
      const moduleMetadata = await Promise.all(
        modules.map(async (module) => {
          const metadata = await this.getModuleMetadata(`${localPath}/${module}.ts`);
        ...
        messages = await tellraw(
          ...
          container([
            ...
            metadata.description
              ? text(`\n  ${metadata.description}`, {style: {color: 'gray'}})
              : text(''),
            text('\n')
          ]).render()
        );
  ...
  @Command(['mag', 'update'])
  @Permission('operator')
  async handleUpdateRepositories({params, kv, tellraw}: ScriptContext): Promise<{ messages: any[] }> {
    let messages = await tellraw(
      params.sender,
  ...
  @Command(['mag', 'repos'])
  @Permission('operator')
  async handleListRepositories({params, kv, tellraw}: ScriptContext): Promise<{ messages: any[] }> {
    let messages = [];
    try {
      const config = await this.getConfig(kv);
      ...
      for (const [repoPath, repo] of repositories) {
        messages = await tellraw(
          ...
          container([
            text(`• ${repoPath}\n`, {style: {color: 'white'}}),
            text(`  Last updated: ${new Date(repo.lastUpdate || 0).toLocaleString()}\n`, {style: {color: 'gray'}}),
            button('View Modules', {
              variant: 'outline',
  ...
  @Permission('operator')
  @Argument([
    { name: "repository", type: "string", description: "GitHub repository to view in format user:repo" }
  ])
  async handleShowRepository({params, kv, tellraw}: ScriptContext): Promise<{ messages: any[] }> {
    let messages = [];
    try {
      const repoPath = params.args.repository as string;
      if (!repoPath?.includes(':')) {
      ...
      for (const module of modules) {
        ...
        if (installedModule) {
          ...
          elements.push(text(` (installed v${moduleInfo.version})`, {style: {color: 'green'}}));
        ...
        if (metadata.description) {
          elements.push(text(`  ${metadata.description}\n`, {style: {color: 'gray'}}));
        }
  ...
  @Permission('operator')
  async handleListInstalled({params, kv, tellraw}: ScriptContext): Promise<{ messages: any[] }> {
    let messages = [];
    try {
      const config = await this.getConfig(kv);
      ...
      for (const [id, module] of installed) {
        messages = await tellraw(
          ...
          container([
            ...
            text(`  Installed: ${new Date(module.installedAt).toLocaleString()}\n`, {style: {color: 'gray'}}),
            module.updatedAt ? text(`  Updated: ${new Date(module.updatedAt).toLocaleString()}\n`, {style: {color: 'gray'}}) : text(''),
            button('Uninstall', {
              variant: 'destructive',
              tooltip: 'Remove this module',
              onClick: {
  ...
  @Permission('operator')
  @Argument([
    { name: "module", type: "string", description: "Module to uninstall in format user:repo[:Module]. If Module is omitted, uninstalls all modules from repository" }
  ])
  async handleUninstall({ params, kv, tellraw }: ScriptContext): Promise<{ messages: any[] }> {
    let messages = await tellraw(
      params.sender,
    ...
    try {
      ...
      if (specificModule) {
        modulesToUninstall = installedModules.filter(m => m.module === specificModule);
        if (modulesToUninstall.length === 0) {
          messages = await tellraw(
            ...
            alert([], {
              variant: 'destructive',
              title: 'Error',
              description: `Module ${specificModule} is not installed from repository ${repoPath}`
            }).render()
          );
          return { messages };
        }
      ...
      messages = await tellraw(
        ...
        container([
          ...
          text(`Successfully uninstalled ${modulesToUninstall.length} module(s)\n`, { style: { color: 'white' } }),
          divider(),
          button('View Installed Modules', {
            variant: 'outline',
  ...
  @Command(['mag', 'backup'])
  @Permission('operator')
  async handleBackup({ params, kv, tellraw }: ScriptContext): Promise<{ messages: any[] }> {
    let messages = await tellraw(
      params.sender,
    ...
    try {
      ...
      messages = await tellraw(
        params.sender,
        container([
          text('Successfully created backup at ', { style: { color: 'green' } }),
  ...
  @Command(['mag', 'restore'])
  @Permission('operator')
  async handleRestore({ params, kv, tellraw }: ScriptContext): Promise<{ messages: any[] }> {
    let messages = await tellraw(
      params.sender,
    ...
    try {
      ...
      for (const [repoPath, repo] of Object.entries(backupConfig.repositories)) {
        try {
          if (!await this.repositoryExists(repoPath)) {
            ...
            messages = await tellraw(
              ...
              container([
                text('\nRestore complete!\n', { style: { color: 'green' } }),
                text(`Repositories: ${Object.keys(backupConfig.repositories).length}\n`, { style: { color: 'white' } }),
      ...
      messages = await tellraw(
        ...
        container([
          text('\nRestore complete!\n', { style: { color: 'green' } }),
          text(`Repositories: ${Object.keys(backupConfig.repositories).length}\n`, { style: { color: 'white' } }),
          text(`Modules: ${Object.keys(backupConfig.installed).length}`, { style: { color: 'white' } })
        ]).render()
      );
  ...
  @Command(['mag', 'upgrade'])
  @Permission('operator')
  async handleUpgrade({params, kv, tellraw}: ScriptContext): Promise<{ messages: any[] }> {
    let messages = await tellraw(
      params.sender,
      ...
      for (const [repoPath, repo] of Object.entries(config.repositories)) {
        try {
          ...
          for (const moduleName of availableModules) {
            ...
            // Get module metadata
            const metadata = await this.getModuleMetadata(`${localPath}/${moduleName}.ts`);
            if (metadata) {
              notInstalled.push({
                moduleId,
                version: metadata.version
              ...
              messages = await tellraw(
                ...
                container([
                  ...
                  text(`No updates needed: ${Object.keys(config.installed).length - updates.length - failed.length} module(s)\n`, {
                    style: {color: 'gray'}
                  }),
