...
export class ModrinthManager {
  ...
  private async updateInstalledMods(
    kv: Deno.Kv,
  ...
  @Permission("operator")
  async mod(
    { params, kv, tellraw, api }: ScriptContext,
  ...
  ): Promise<{ messages: any[]; results?: ModrinthProject[] }> {
      ...
      for (const hit of results.hits.slice(0, 5)) {
        ...
        messages = await tellraw(
          ...
          JSON.stringify([
            ...
            {
              text: `Categories: ${project.categories.join(", ")}`,
              color: "aqua",
            },
          ]),
        );
  ...
  async install({
    ...
    files,
    sendToMinecraft,
  }: ScriptContext): Promise<
    { messages: any[]; success?: boolean; installedMod?: InstalledMod }
  > {
    const { sender, args } = params;
    const { slug } = args;
    const mcVersion = await this.getServerVersion(kv);
    ...
    try {
      ...
      const versionsResponse = await this.fetchFromModrinth(
      ...
      if (!version) {
        throw new Error(
          `No compatible version found for Minecraft ${mcVersion}`,
        );
      }
  ...
  async list(
    { params, tellraw, log, kv }: ScriptContext,
  ): Promise<{ messages: any[]; mods?: InstalledMod[] }> {
    const { sender } = params;
    let messages = [];
  ...
  @Permission("operator")
  async update(
    { params, tellraw, log, kv, files, sendToMinecraft }: ScriptContext,
  ): Promise<{
    messages: any[];
    success?: boolean;
    updatedCount?: number;
        ...
        try {
          ...
          if (latestVersion.version_number !== mod.version) {
            ...
            messages = await tellraw(
              ...
              JSON.stringify({
                text:
                  `Updated ${mod.title} from v${mod.version} to v${latestVersion.version_number}`,
                color: "green",
              }),
            );
