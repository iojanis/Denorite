...
export class RegistryHelper {
  ...
  async handleGetPackages({params, log, kv}: ScriptContext): Promise<{
    ...
    try {
      ...
      if (!type || type === 'module') {
        ...
        if (moduleResp.ok) {
          ...
          for (const mod of moduleData) {
            ...
            const fullModuleResp = await fetch(`${config.registryUrl}/${mod.userspace || 'public'}/modules/${mod.name}`, {
              headers: {
                'Authorization': `Bearer ${config.serverToken}`
              }
            });
            ...
            if (fullModuleResp.ok) {
              ...
              modules.push({
                ...
                updateAvailable: installedVersion && installedVersion !== mod.version
              });
            }
          }
        }
      ...
      if (!type || type === 'component') {
        const componentResp = await fetch(`${config.registryUrl}/public/components`, {
          headers: {
            'Authorization': `Bearer ${config.serverToken}`
          }
        });
        ...
        if (componentResp.ok) {
          ...
          for (const comp of componentData) {
            ...
            const fullCompResp = await fetch(`${config.registryUrl}/${comp.userspace || 'public'}/components/${comp.name}`, {
              headers: {
                'Authorization': `Bearer ${config.serverToken}`
              }
            });
            ...
            if (fullCompResp.ok) {
              ...
              components.push({
                ...
                type: 'component',
                isInstalled: !!installedVersion,
                updateAvailable: installedVersion && installedVersion !== comp.version
              });
            }
          }
        }
      ...
      log(`Retrieved ${modules.length} modules and ${components.length} components`);
  ...
  async handleInstall({params, log, kv}: ScriptContext): Promise<void> {
    try {
      ...
      // Parse path
      const [userspace, ...nameParts] = path.includes('/') ? path.split('/') : [config.defaultUserspace, path];
      ...
      const response = await fetch(`${config.registryUrl}/${userspace}/${type}s/${name}`, {
        headers: {
          'Authorization': `Bearer ${config.serverToken}`
        }
      });
  ...
  async handleUpdate({params, log, kv}: ScriptContext): Promise<void> {
    try {
      ...
      const response = await fetch(`${config.registryUrl}/${record.userspace}/${type}s/${name}`, {
        headers: {
          'Authorization': `Bearer ${config.serverToken}`
        }
      });
      ...
      if (pkg.metadata.version === record.version) {
        throw new Error(`Package ${name} is already up to date (v${record.version})`);
      }
